...
// Côté client avec interface ncurses. Utiliser la commande ./c_interface <Username> 127.0.0.1 8080
...
char name[32];
char eff[]="                                                                                                                                                                                                                                                                                                             ";
char mesg[] = "Ecrire un message : ";
int i=4;
int boite_x;


/**
 * Créé une interface simple avec champ d'écriture et les messages qui s'affichent.
*/
void interface(){

    start_color();	
    init_pair(1, COLOR_WHITE, COLOR_BLACK);
    boite_x = LINES-(LINES/7);
    boite= subwin(stdscr, LINES/7, COLS, boite_x , 0);
    box(boite, ACS_VLINE, ACS_HLINE); 
    attron(COLOR_PAIR(1));
    
    attron(A_UNDERLINE | A_BOLD);
    mvprintw(1, (COLS/2)-18, "Bienvenue dans le salon de dicussion");
    attroff(A_UNDERLINE | A_BOLD);

    // permet d'ecrire dans la box
    attron(A_NORMAL);
    mvprintw(boite_x - 1, 1,"%s",mesg);
    attroff(A_NORMAL);       
...
void send_msg(){
  ...
  while(1) {
    ...
    // permet d'ecrire le message
    mvgetstr(boite_x + 1, 1 ,buffer);
    mvprintw(boite_x + 1, 1, " %s",eff);
    box(boite, ACS_VLINE, ACS_HLINE);
...
void recv_msg(){
  ...
  while (1) {
    ...
    if (recv > 0) {
        // efface ancien message pour pas deborder dans la zone d'ecriture
        if (i == boite_x - 3){ 
            i = 4;
            clear();
            interface();
        ...
        mvprintw(i, 1, "%s",message);
        i++;
        mvprintw(boite_x +1, 1, "");
        refresh();
